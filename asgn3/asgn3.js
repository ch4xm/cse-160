...
function addWalls(map, height = 1) {
  for (x = 0; x < map.length; x++) {
    for (y = 0; y < map[x].length; y++) {
      if (x == 0 || x == map.length - 1 || y == 0 || y == map[x].length - 1) {
        // console.log('a', Array.from(Array(height).keys()))
        map[x][y] = [1,2,3]
        // map[x][y] = Array.from(Array(height).keys())
        // map[x][y] = Array.from(new Set([...map[x][y], ...new Set([...Array(height).keys()])]))
      } else {
        // map[x][y] = Array.from(new Set(map[x][y]).union(new Set()))
        // map[x][y] = Array.from(new Set(map[x][y]))
      }
    }
  }
  // return map
  // console.log(map, 'adad')
}
...
//     for (y = 0; y < map[x].length - 1; y++) {
//       for (let i = 0; i < map[x][y]; i++) {
...
function drawMap(map) {
  for (x = 0; x < map.length; x++) {
    for (y = 0; y < map[x].length; y++) {
      // console.log(map[x][y].length, x, y, map[x][y])
      for (let i = 0; i < map[x][y].length - 1; i++) {
          if (map[x][y] = 4) {
            console.log('fingongrjg')
          }
          const cube = new Cube();
          cube.textureNum = SKY_TEXTURE;
          cube.color = [0.5, 0.5, 0.5, 1];
